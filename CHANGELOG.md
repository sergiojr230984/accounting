...
## [Unreleased] — Invoice/estimate list ordering (2026-08-29)
...
- Customer invoices (`GET /api/invoices/customer`) and estimates
  (`GET /api/estimates`) now sort by `invoiceNumber`/`estimateNumber`
  descending. Both are generated by `lib/next-number.ts`'s
  `formatSequenceNumber()` as a fixed prefix plus a zero-padded sequence,
  so sorting the string is equivalent to sorting the number — no jumbling.
- Supplier bills (`GET /api/invoices/supplier`) sort by `createdAt`
  descending instead. Deliberately **not** switched to `invoiceNumber`
  like the other two: a bill's `invoiceNumber` is the supplier's own
  free-text invoice number (see `SupplierInvoice` in `schema.prisma`), not
  one this system generates sequentially, so sorting it alphabetically
  wouldn't produce chronological order and could jumble things worse than
  dates did.
